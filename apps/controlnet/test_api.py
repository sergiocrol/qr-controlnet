...
def main():
    ...
    parser.add_argument('--device', default='cpu', choices=['auto', 'cpu', 'mps'], help='Device to use')
    ...
    if not os.path.exists(args.image):
        ...
        try:
            print("Creating a sample QR code...")
            import qrcode
            qr = qrcode.QRCode(
                version=1,
            ...
            img.save(args.image)
            print(f"Created sample QR code at {args.image}")
        except ImportError:
            print("QRCode library not installed. Please install with: pip install qrcode")
            return
        except Exception as e:
            print(f"Error creating sample QR code: {e}")
            return
    ...
    try:
        ...
        print(f"Request timed out after {args.timeout} seconds. The server is likely still processing.")
        print("Try increasing the timeout with --timeout parameter or reduce model complexity.")
    except requests.exceptions.ConnectionError as e:
        print(f"Connection error: {e}")
        print("The server might be overloaded or crashed. Check the container logs.")
    except Exception as e:
        print(f"Error: {e}")
